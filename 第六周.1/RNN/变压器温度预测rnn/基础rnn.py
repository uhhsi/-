...
class RNNModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        ...
        # num_layers: RNN堆叠层数 (例如2)
        # batch_first: 输入/输出张量形状为 (batch, seq_len, features)
        self.rnn = nn.RNN(input_size, hidden_size, num_layers, batch_first=True)
        # 数据变化：定义了一个可处理 (batch, seq_len, 4) -> (batch, seq_len, 64) 的RNN层

        # 初始化全连接层
        # hidden_size: 输入维度 (RNN输出的隐藏状态维度)
    ...
    def forward(self, x):
        ...
        # 初始化隐藏状态h0
        # self.rnn.num_layers: RNN层数 (例如2)
        # x.size(0): 当前batch大小 (例如32)
        # self.rnn.hidden_size: 隐藏层维度 (例如64)
        h0 = torch.zeros(self.rnn.num_layers, x.size(0), self.rnn.hidden_size).to(x.device)
        # h0形状: (num_layers=2, batch_size=32, hidden_size=64)
        # 数据变化：创建全零初始隐藏状态
        ...
        # _形状: (2, 32, 64)     (最终隐藏状态，此处被丢弃)
        # 数据变化：RNN处理时序数据，输出每个时间步的隐藏状态
